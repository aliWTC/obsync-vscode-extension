...
exports.updateFunctionGraphForFile = updateFunctionGraphForFile;
exports.pruneFunctionGraph = pruneFunctionGraph;
exports.recalculateCallers = recalculateCallers;
function updateFunctionGraphForFile(index, analysis, relativePath, version) {
    ...
    for (const def of analysis.definitions) {
        ...
        const existingDefinition = definitions.find((item) => item.relativePath === relativePath &&
            item.kind === def.kind &&
        ...
        const filteredDefinitions = definitions.filter((item) => !(item.relativePath === relativePath && item.kind === def.kind && item.lineStart === def.lineStart));
        const record = {
            id: functionId,
            name: functionNameFromId(functionId),
            definitions: [...filteredDefinitions, nextDefinition].sort((a, b) => `${a.relativePath}:${a.lineStart}`.localeCompare(`${b.relativePath}:${b.lineStart}`)),
    ...
    for (const staleId of previousIds) {
        ...
        const remainingDefinitions = stale.definitions.filter((item) => item.relativePath !== relativePath);
    ...
    next.fileFunctionIds[relativePath] = Array.from(new Set(nextIds)).sort((a, b) => a.localeCompare(b));
    next.fileCalls[relativePath] = Array.from(new Set(analysis.calls.map((call) => call.calleeName).filter(Boolean)));
    return recalculateCallers(next);
}
function pruneFunctionGraph(index, keepRelativePaths) {
    const next = cloneIndex(index);
    ...
    for (const [functionId, record] of Object.entries(next.functionsById)) {
        const remainingDefinitions = record.definitions.filter((item) => keepRelativePaths.has(item.relativePath));
        if (remainingDefinitions.length === 0) {
...
function recalculateCallers(index) {
    const next = cloneIndex(index);
    next.functionsById = {};
...
function functionNameFromId(id) {
